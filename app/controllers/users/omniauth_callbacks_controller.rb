...
class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  ...
  def facebook
    ...
    if !current_user.blank?
      ...
      redirect_to workspace_settings_path, :notice => "Bind facebook account successfully."
    else
      @user = User.find_or_create_for_provider(provider, uid, data, token)
  ...
  def github
    ...
    if !current_user.blank?
      ...
      redirect_to workspace_settings_path, :notice => "Bind Github account successfully."
    else
      @user = User.find_or_create_for_provider(provider, uid, data, token)
      sign_in_and_redirect @user, :event => :authentication, :notice => "Signed in successfully."
    end
